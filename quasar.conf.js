...
module.exports = function (/* ctx */) {
  ...
  return {
    ...
    build: {
      ...
      extendWebpack (cfg) {
        cfg.plugins.push(new CompressionWebpackPlugin({
          filename: process.env.APP_ENV !== 'local' ? '[path][base]' : '[path][base].gz',
          algorithm: 'gzip',
          test: /\.(js|svg)$/,
          threshold: 0,
          minRatio: 1,
          deleteOriginalAssets: process.env.APP_ENV !== 'local'
        }))
    ...
    pwa: {
      ...
      manifest: {
        name: 'Aloware Talk',
        short_name: 'Aloware Talk',
        description: 'New Aloware Agent Interface',
        display: 'standalone',
