export default {
  check: async ({ commit }, preventLogout = false) => {
    try {
  ...
  getCookieUser: async ({ commit }) => {
    try {
      commit('SET_LOADING', true)

      let cookieParams = { shared_token: this.getSharedCookie() }
  ...
  resetPass: async ({ commit }, payload) => {
    try {
  ...
  impersonate: async ({ commit }, payload) => {
    try {
