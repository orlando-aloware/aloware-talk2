...
export default {
  ...
  methods: {
    ...
    stateToIcon (dispositionStatus, direction, type, callbackStatus, onlyColor = false) {
      let color = ''
      let icon = ''
      if (type === 1 || type === 3) {
        if (dispositionStatus === CommunicationDispositionStatus.DISPOSITION_STATUS_INPROGRESS_NEW) {
          ...
          if (direction === CommunicationDirection.INBOUND && [CallbackStatus.CALLBACK_STATUS_INITIATED, CallbackStatus.CALLBACK_STATUS_REQUESTED].includes(callbackStatus)) {
