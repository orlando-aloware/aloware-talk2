...
export default {
  ...
  methods: {
    checkCommunicationMatchesUserAccessibility (communication) {
      ...
      if (this.auth.user.profile.communications_visibility === CommunicationAccessTypes.COMMUNICATIONS_OWNED_ONLY && communication.used_id && communication.used_id !== this.auth.user.profile.id) {
        return false
      }

      // focus mode
      if (this.auth.user.profile.focus_mode) {
