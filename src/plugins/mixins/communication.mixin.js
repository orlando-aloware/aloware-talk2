...
export default {
  ...
  methods: {
    checkCommunicationMatchesUserAccessibility (communication) {
      ...
      if (this.user.profile.accessible_campaigns && this.user.profile.line_access_limit && communication.campaign_id && !this.user.profile.accessible_campaigns.includes(communication.campaign_id)) {
        return false
      }

      // checks if accessible_users is available and then looks for communication user_id in that array
      if (this.user.profile.accessible_users && this.user.profile.user_access_limit && communication.user_id && !this.user.profile.accessible_users.includes(communication.user_id)) {
        return false
      }

      // checks if accessible_users is available and then looks for an intersection between accessible_users and attempting_users
      if (this.user.profile.accessible_users && this.user.profile.user_access_limit && communication.attempting_users && [CommunicationCurrentStatus.CURRENT_STATUS_TRANSFERRING_NEW, CommunicationCurrentStatus.CURRENT_STATUS_GREETING_NEW].includes(communication.current_status2) && _.intersection(this.user.profile.accessible_users, communication.attempting_users).length === 0) {
        return false
      }

      // checks if communication matches user communication visibility
      if (this.user.profile.communications_visibility === CommunicationAccessTypes.COMMUNICATIONS_OWNED_ONLY && communication.used_id && communication.used_id !== this.user.profile.id) {
        return false
      }

      // focus mode
      if (this.user.profile.focus_mode) {
        // checks if communication's contact is owned by the user
        if (communication.contact && communication.contact.user_id && communication.contact.user_id !== this.user.profile.id) {
          return false
        }
      ...
      if (this.user.profile.contacts_visibility === ContactAccessTypes.CONTACTS_ACCESS_RING_GROUP) {
        // if user does not have unassigned access
        if (communication.contact && !communication.contact.user_id && !this.user.profile.can_view_unassigned_contacts) {
          return false
        }
      ...
      // owned only access
      if (this.user.profile.contacts_visibility === ContactAccessTypes.CONTACTS_ACCESS_OWNED_ONLY) {
        // if user does not have unassigned access
        if (communication.contact && !communication.contact.user_id && !this.user.profile.can_view_unassigned_contacts) {
          return false
        }
      ...
      if (this.user.profile.company_id === 11 && this.hasRole('Company Reporter Access')) {
        ...
        if (communication.contact.user_id && communication.contact.user_id !== this.user.profile.id) {
          return false
        }

        // checks if communication matches user visibility
        if (communication.owner_id && communication.owner_id !== this.user.profile.id) {
          return false
        }
      }
    ...
    checkContactMatchesUserAccessibility (contact) {
      // check auth exists to prevent js errors
      if (!this.user || !this.user || !this.user.profile) {
        return false
      }
