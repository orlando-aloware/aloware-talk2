  ...
  <div class="row no-wrap pt-3 pb-3 width-380 dialer-wrapper"
    ...
    <div class="col phone-padding dialer-tabs-wrapper"
      ...
      <b-tabs class="dialer-tabs"
              pills
              vertical>
        <block-tooltip placement="bottomleft"
                       triggers="hover focus"
                       target="calls-popover"
                       task="calls">
        </block-tooltip>
        <block-tooltip placement="bottomleft"
                       triggers="hover focus"
                       target="texts-popover"
                       task="texts">
        </block-tooltip>
        <b-tab title="Call"
               :active="mode === 'call'"
    ...
    async changePhoneNumber (data) {
      ...
      if (!this.contactId) {
        ...
        await this.getContactByPhoneNumber(this.phoneNumber).then((data) => {
          ...
          /* eslint-disable */console.log(...oo_oo(`726803025_0`,err))
          this.loadingContact = false
        })
      }
      ...
      this.$axios.post('/api/v1/campaign/send-message-to-phone-number/' + this.campaignId, {
        ...
        if (!this.isMobile) {
          this.$router.push({
            ...
            /* eslint-disable */console.log(...oo_oo(`726803025_1`,err))
          })
        }
      }).catch(err => {
        this.isSending = false
