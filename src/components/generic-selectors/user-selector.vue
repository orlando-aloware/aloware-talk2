    ...
    <template v-slot:option="scope">
      ...
      >
        <q-item-section>
          ...
          <q-item-label caption v-if="!scope.opt.is_destination">
      ...
      <q-item
        ...
        v-on="scope.itemEvents"
      >
        <q-item-label header class="text-size-xs">{{
          scope.opt.group
        }}</q-item-label>
      </q-item>
    </template>

    <template v-slot:selected-item="scope" v-if="useChips">
      <q-chip
        color="white"
      ...
      >
        <i
          class="fa fa-circle position-absolute"
        ...
        >
          <remove-tag-icon
            class="ml-1 remove-tag-icon"
            data-testid="user-selector-select-remove-tag-icon"
          />
        </div>
      </q-chip>
    </template>
    ...
    filteredUsers () {
      if (!_.isEmpty(this.availableUsers)) {
        return this.availableUsers.filter(
          (user) =>
            !(
              (typeof user.role_names === 'undefined' ||
  ...
  methods: {
    filterFn (val, update) {
      if (this.selectedId && val === this.selectedId) {
        update(() => {
          this.userOptions = this.formattedOptions.filter(
