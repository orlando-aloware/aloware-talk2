...
export default {
  ...
  methods: {
    ...
    loadAllCount () {
      ...
      return this.$axios.get(`https://pr-10484.mde.alodev.org/api/v2/contacts/count`).then((response) => response.data.count)
    },

    loadMyContactsCount () {
      // return this.$axios.get(`${process.env.API_REPORTING_URL}/api/v2/contacts/count`, {
      return this.$axios.get(`https://pr-10484.mde.alodev.org/api/v2/contacts/count`, {
        params: {
          filter_groups: [{
            filters: {
              contact_owner: [
    ...
    loadStatusCounts () {
      return this.$axios.get(`https://pr-10484.mde.alodev.org/api/v2/contacts/status-counts`).then((response) => response.data)
        .catch((err) => {
          console.error(err)
