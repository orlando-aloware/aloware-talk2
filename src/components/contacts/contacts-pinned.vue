...
export default {
  ...
  methods: {
    ...
    loadAllCount () {
      ...
      return this.$axios.get(`${process.env.API_URL}/api/v2/contacts/count`).then((response) => response.data.count)
    },
    ...
    loadStatusCounts () {
      ...
      return this.$axios.get(`${process.env.API_URL}/api/v2/contacts/status-counts`).then((response) => response.data)
        .catch((err) => {
          console.error(err)
