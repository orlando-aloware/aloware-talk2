      ...
      <q-card-section>
        <b-row>
          <b-button class="text-white"
            ...
            {{ isSyncing ? 'Syncing...' : 'Sync with Zoho' }}
...
import { zohoIntegrationMixin, integrationMixin } from 'src/plugins/mixins'
...
export default {
  ...
  computed: {
    ...
    zohoContactLink () {
      ...
      return this.zohoContactLink(this.contact)
    }
  },
  ...
  methods: {
    ...
    getData () {
      return this.getIntegrationData(this.contact, 'zoho')
        .then(response => {
