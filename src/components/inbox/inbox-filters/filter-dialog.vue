<template>
  ...
  >
    ...
    >
      <div class="left-column-wrapper" v-if="!isFilterDialogForView">
        <span class="filter-type-description">{{ channelFilterName }}</span>
        <div class="mt-2">
          <div class="left-column-wrapper-scrollable">
            <div class="mb-4">
              <div
                class="filter-items cursor-pointer position-relative"
              ...
              >
                <div>
                  ...
                  <span
                    class="position-absolute check-icon"
                    v-if="!selectedFilter"
                  >
                    <check-o-icon
                      data-testid="filter-dialog-check-o-icon"
                      color="#040404"
                    />
                  </span>
                </div>
            ...
            <h5
              class="text-uppercase filter-group-title"
            ...
            <div class="saved-filters">
              ...
              />
              <p
                class="text-muted fs-12 empty-filter-placeholder pl-2"
            ...
            <div class="saved-filters">
              ...
              <div
                class="filter-items cursor-pointer"
      ...
      >
        <div class="d-flex justify-content-between mb-3 px-3">
          <q-input
            ...
            v-if="
              isEditingView &&
              selectedFilter &&
              (!+selectedFilter?.is_on_company ||
                selectedFilter?.scope === 'user')
            "
            @keyup.enter="renameFilter"
          />
          <div class="w-100 text-left pt-2 pb-1" v-else>
            <span class="filter-name">{{ filterFormDisplayName }}</span>
          </div>
          <compact-btn
            class="border-0 pl-0 pr-0"
        ...
        <div class="d-flex justify-content-end mt-sm-3 px-3">
          <div>
            <compact-btn
              class="mr-2 btn-outline-primary"
            ...
            >
              ...
              >
                <span
                  >After clicking the <strong>Reset</strong> button, please make
                  sure to click <strong>Apply</strong> to confirm the
                  changes.</span
                >
              </q-tooltip>
              Reset
...
export default {
  ...
  mounted () {
    ...
    this.$VueEvent.listen('channel_filter_created', (filter) => {
      ...
      const exists = this.inboxPersonalFilters.find(
  ...
  computed: {
    ...
    channelFilterName () {
      const isInbox =
        !this.$route.params.channel && this.$route.name === 'Inbox'

      if (
        this.isFilterDialogForView ||
        ['Inbox View', 'Inbox View Contact Task'].includes(this.$route.name)
      ) {
        return 'Views'
      }
    ...
    filterHasChanges () {
      const filterIdentifier = this.isFilterUpdateMode
        ? this.selectedFilter.filter
        : this.loadedDefaultFilterModel.filter

      for (const field of this.filterFields) {
        if (
          JSON.stringify(this.filter[field]) !==
          JSON.stringify(filterIdentifier[field])
        ) {
          return true
        }
      }
    ...
    applyButtonText () {
      ...
      const userScope =
        this.selectedFilter?.scope === 'user' ||
        +!this.selectedFilter?.is_on_company
    ...
    isSaveAsNewDisabled () {
      return (
        this.isNonViewCreateModeUnchanged ||
    ...
    isViewEditModeOrNonView () {
      return (
        !this.isFilterDialogForView ||
        (this.isFilterDialogForView && this.isEditingView)
      )
    },

    isFilterUpdateMode () {
      return (
        (!this.isFilterDialogForView && this.selectedFilter) ||
        (this.isFilterDialogForView && this.isEditingView)
      )
    },
  ...
  methods: {
    ...
    onHidden () {
      const isForCreateView =
        this.isFilterModelFormShown && this.isFilterDialogForView
      ...
      if (
        !this.isFilterDialogForView &&
        !this.appliedFilter &&
        this.channelChangedFilterFields.length
      ) {
        this.setSelectedFilter(null)
      }
    ...
    onShow () {
      ...
      if (!this.filter.hasOwnProperty('dynamic_engagement_date_range')) {
        this.filter.dynamic_engagement_date_range =
          Filters.DEFAULT_STATE.filter.dynamic_engagement_date_range
      }
    ...
    onResetFilter () {
      ...
      const useFilter =
        this.selectedFilter && this.isFilterDialogForView && this.isEditingView
          ? this.selectedFilter.filter
    ...
    onApply (skipChangedFields = false) {
      ...
      if (
        myContactsFilter !== null &&
        myContactsFilter !== (this.inboxShowMyContacts | 0)
      ) {
      ...
      if (
        unreadsFilter !== null &&
        unreadsFilter !== (this.inboxShowUnreads | 0)
      ) {
      ...
      const excludeProps = ['my_contact']

      if (!skipChangedFields) {
        for (const item in this.filter) {
          if (
            item === 'answer_status' &&
            this.loadedDefaultFilterModel.type ===
              ChannelType.CHANNEL_RECORDINGS
          ) {
            continue
          }

          const hasField =
            this.filterFields.includes(item) &&
            this.loadedDefaultFilterModel.filter.hasOwnProperty(item)

          // for boolean fields change tracking
          if (
            booleanProps.includes(item) &&
            +this.filter[item] !==
              +this.loadedDefaultFilterModel.filter[item] &&
            hasField
          ) {
          ...
          const loadedFilterItem = JSON.stringify(
            this.loadedDefaultFilterModel.filter[item]
          )
          if (
            !this.booleanFields.includes(item) &&
            filterItem !== loadedFilterItem &&
            hasField &&
            !excludeProps.includes(item)
          ) {
            this.updateChannelChangedFilterFields({
              name: item,
              value: this.filter[item]
      ...
      const userScope =
        this.selectedFilter?.scope === 'user' ||
        !+this.selectedFilter?.is_on_company
      const hasSelectedFilterChanges =
        this.selectedFilter && userScope && this.filterHasChanges
      ...
      if (
        [
          ChannelType.CHANNEL_RECORDINGS,
          ChannelType.CHANNEL_VOICEMAILS
        ].includes(this.defaultFilterModel.type) &&
    ...
    getFilters () {
      ...
      const type =
        this.defaultFilterModel.type === ChannelType.CHANNEL_RECORDINGS
    ...
    getTagsByIds (ids) {
      return talk2Api.V1.tags
        .get({ params: { tag_ids: ids } })
        .then((response) => {
    ...
    updateFilter (filter, params) {
      if (!params.scope) {
        const scope = [1, '1', true].includes(filter.is_on_company)
          ? 'company'
          : 'user'
        params = {
          ...params,
      ...
      if (
        this.loadedDefaultFilterModel.type === ChannelType.CHANNEL_RECORDINGS
      ) {
      ...
      return talk2Api.V2.inbox.filters.update(filter.id, params).then((res) => {
        ...
        if (!updatedFilter.is_on_company) {
          const personalFiltersIndex = this.personalFilters.findIndex(
            (item) => item.id === filter.id
          )
          const inboxPersonalFiltersIndex = this.inboxPersonalFilters.findIndex(
            (item) => item.id === filter.id
          )
          const pinnedViewIndex = this.pinnedViews.findIndex(
            (view) => +view.filter_id === +filter.id
          )

          // update personal filters based on actively selected channel
          if (
            personalFiltersIndex >= 0 &&
            updatedFilter.type === this.loadedDefaultFilterModel.type
          ) {
          ...
          if (
            inboxPersonalFiltersIndex >= 0 &&
            updatedFilter.type === ChannelType.CHANNEL_INBOX
          ) {
    ...
    onDeleteFilter (filter) {
      ...
      return talk2Api.V2.inbox.filters.delete(filter.id).then((res) => {
        ...
        if (!filter.is_on_company) {
          ...
          if (filter.type === this.defaultFilterModel.type) {
            this.personalFilters = this.personalFilters.filter(
              (item) => item.id !== filter.id
            )
          }
          ...
          if (filter.type === ChannelType.CHANNEL_INBOX) {
            const inboxPersonalFilters = this.inboxPersonalFilters.filter(
              (item) => item.id !== filter.id
            )
    ...
    applyFilter () {
      const isCreateViewWithActiveFilter =
      ...
      this.filter = {
        ...
        exclude_automated_communications:
          +this.filter.exclude_automated_communications,
    ...
    getFilterItemClass (item) {
      const selectedFilterClass =
        this.selectedFilter?.id === item.id ? 'active' : ''
