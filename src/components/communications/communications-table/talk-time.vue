  ...
  <div>
    <div class="row"
      ...
      <div class="col-12 d-flex align-items-center justify-content-left">
        <span
          v-if="row.disposition_status2 !== CommunicationDispositionStatus.DISPOSITION_STATUS_INPROGRESS_NEW && row.current_status2 === CommunicationCurrentStatus.CURRENT_STATUS_COMPLETED_NEW">
          {{ row.talk_time | fixDuration }}
        </span>
        <relative-time class="text-muted"
                       data-testid="comm-log-relative-time"
                       :from_time="row.created_at"
    ...
    <div class="row">
      <div class="col-12 d-flex align-items-center justify-content-left">
        ...
        <span v-else>{{ getVisibleStatus(row) }}</span>
      </div>
    </div>
  </div>
</template>
