  ...
  <q-toolbar class="page-header"
    ...
    <div class="d-flex h-100 align-items-center flex-grow-1">
      <back-button class="mobile-back-btn-global-header"
                   v-if="['Contact', 'Settings Tab', EINBOXES_MENU_ITEMS, EINBOXES_MENU_COMMUNICATIONS].includes($route.name)"
                   @click="navigateBack"/>
...
import { COMMUNICATIONS_CHANNELS_ROUTE_NAME, EINBOXES_MENU_ITEMS, EINBOXES_MENU_COMMUNICATIONS } from 'src/router/routes'
...
export default {
  ...
  methods: {
    ...
    navigateBack (e) {
      ...
      if (['Settings Tab', EINBOXES_MENU_ITEMS, EINBOXES_MENU_COMMUNICATIONS].includes(this.$route.name)) {
        this.$router.back()
        return
      }
