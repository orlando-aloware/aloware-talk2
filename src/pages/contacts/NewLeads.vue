<template>
  <contacts-screen :loading="isLoadingDisabled">
    <template slot="title">
      <div class="pr-2">New Leads</div>
      <span class="small text-muted"
        >{{ newleads.total_contact_count }} contacts found</span
      >
    </template>
    ...
    <template slot="table">
      ...
      >
        <table-row
          v-for="(contact, index) in newleads.data"
...
export default {
  ...
  methods: {
    ...
    onLoadMore () {
      if (this.hasMore) {
        ...
        const nextPage = this.newleads.current_page + 1
        this.fetchContacts({
          page: nextPage,
          search_text: this.searchText
        })
          .then((data) => {
            this.contactsLoaded({ type: 'newleads', append: true, ...data })
          })
          .finally(() => {
    ...
    fetch (params = {}) {
      ...
      Promise.all([this.fetchContacts(params), this.fetchContactsCount(params)])
        .then(([data, count]) => {
          this.contactsLoaded({ type: 'newleads', append: false, ...data, ...count })
        })
        .finally(() => {
  ...
  computed: {
    ...
    ...mapGetters('contacts', ['newleads']),
    hasMore () {
      return (
        this.newleads.next_page_url && !this.isLoadingMore && !this.isLoading
      )
    },
    isLoadingDisabled () {
    ...
    isEmpty () {
      return this.isLoaded && !this.newleads.data.length
    }
  },
  data () {
    return {
