    ...
    <div class="clear d-flex flex-column"
      ...
      <el-badge class="item px-2 d-flex flex-column"
        ...
        <div class="px-2"
             ...
             :key="index">
          <vue-load-image v-if="isAttachmentImage(attachment.mime_type)">
            <img slot="image"
...
export default {
  ...
  computed: {
    ...mapState(['campaigns', 'workflows', 'broadcasts', 'disposition_statuses', 'call_dispositions']),
  ...
  methods: {
    ...
    customAuditsConditions(data) {
      ...
      switch (property) {
        ...
        case 'thread_status':
          ...
          return allowed_data.includes(property_value[0]) ||
            allowed_data.includes(property_value[1])
        default:
          return false
      }
    },
    ...
    generalAuditMessages(data) {
      ...
      let to_value = ''
      if (data.from) {
        from_value = data.property === 'disposition_status_id' ? this.getContactDisposition(data.from).name : ''
        from_value = data.property === 'user_id' ? this.getUser(data.from).name : from_value
        from_value = !['disposition_status_id', 'user_id'].includes(data.property) ? data.from : from_value
      }
      if (data.to) {
        to_value = data.property === 'disposition_status_id' ? this.getContactDisposition(data.to).name : ''
        to_value = data.property === 'user_id' ? this.getUser(data.to).name : to_value
        to_value = !['disposition_status_id', 'user_id'].includes(data.property) ? data.to : to_value
      }
      if (!data.from && data.to) {
        switch (data.property) {
          case 'disposition_status_id':
          case 'user_id':
            return general_message += ' has been set to "' + to_value + '"'
          case 'workflow_id':
            return workflow_message[0]
        }
      }
      if (data.from && data.to && data.property !== 'workflow_id') {
        return general_message += ' has been changed from "' + from_value + '" to "' + to_value + '"'
      }
      if (data.from && !data.to) {
        switch (data.property) {
          case 'disposition_status_id':
          case 'user_id':
            return general_message += ' has been removed from "' + from_value + '"'
          case 'workflow_id':
